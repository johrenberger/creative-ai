...
// Closes: CTA-GAP-005 (server.js's duplicate parseSessionCookie coverage)
// Source: src/server.js
//
// NOTE: src/server.js has its own parseSessionCookie function at line ~105,
// duplicated from src/middleware/auth.js. The recommended path is to refactor
// to use the middleware version. This test adds coverage of the duplicate
// to raise server.js coverage, but the refactor is logged as a follow-up
// in TODO_test-generation.md and should be done in a separate change.
// DO NOT EDIT this header — the generator relies on it for audit.
// ============================================================

// (jest, describe, it, expect, beforeAll, afterAll, app, testSessionCookie already in scope above)

describe('server.js parseSessionCookie (auto: CTA-GAP-005)', () => {
  it('GET /api/auth/session returns authenticated:false when cookie has no session_id', async () => {
  ...
  it('GET /api/auth/session returns authenticated:true with valid session_id cookie', async () => {
    const res = await request(app)
      .get('/api/auth/session')
  ...
  it('GET /api/auth/session returns authenticated:false and clears cookie for expired token', async () => {
    const dbModule = await import('../src/db.js');
    const db = dbModule.getDb();
    const sessionToken = testSessionCookie.match(/session_id=([^;]+)/)[1];
    const pastIso = new Date(Date.now() - 60_000).toISOString();
    db.prepare("UPDATE sessions SET expires_at = ? WHERE session_token = ?").run(pastIso, sessionToken);
    ...
    expect(res.headers['set-cookie'].join(';')).toMatch(/session_id=;/);
  });

  it('POST /api/auth/logout is a no-op when no session_id cookie is present', async () => {
    const res = await request(app)
  ...
  it('POST /api/auth/logout destroys the session and returns success', async () => {
    ...
    const logoutRes = await testAgent
      .post('/api/auth/logout')
      .set('Cookie', cookie);
    ...
    const sessionCheck = await testAgent
      .get('/api/auth/session')
      .set('Cookie', cookie);
